...
| W1.27 | Partner logo assets wired into `partner-row` — **DONE** (`partner-row.items[].logo` image; RHD/SDIG-SRBG/UDC/DBEDC marks registered in `media` and wired on home by 23-partner-logos.sql) | OP-4, UI-8 | `public/brand/{rhd,udc,sdig-srbg,dbedc}.{png,webp}`, `lib/blocks/types/partner-row.js` | Replace the typographic-credit fallback with the real marks. **Light surfaces only** — see UI-8. Update the block's header comment, which currently states no vector marks are held. |
