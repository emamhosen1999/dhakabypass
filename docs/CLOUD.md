...
| Institutional pages | `lib/institutional/pages.js`, seeded by `scripts/seed-institutional.mjs` — CMS rows, edited in the admin |
| Newsroom | `lib/newsroom/`, `news_translations` (v7); translations edited at `/admin/news/<id>/translations` |
| Gallery | `lib/gallery/`, `media.in_gallery` (v8, default 0); toggled per image in `/admin/media` |
| Analytics | `lib/analytics/config.js` — off unless `ANALYTICS_PROVIDER` is set; `ga4` also switches on a consent banner |
| Migration SQL | `db/sql/` — GENERATED by `npm run db:sql`, imported in phpMyAdmin |
| Plans | `docs/superpowers/plans/` |
| Deployment | `docs/deployment/` — read the readiness review before deploying |

## The state of the deployment

**There is a working deploy path.** It is written down in
`docs/deployment/2026-09-04-deploy-runbook.md`, which is the single source of
truth — three plan documents describe an older contract and now carry a banner
pointing at it.

The model: build here, push the built artifact to an orphan `deploy/*` branch,
`git pull` on the server. No SSH at deploy time, no folder copying on the host,
no `npm install` and no `next build` there. Database migrations are the only
manual step and are SQL imported through phpMyAdmin (`db/sql/`, generated by
`npm run db:sql`, and shipped inside the release).

```bash
SITE_URL=https://staging.dhakabypass.com npm run build   # postbuild assembles + verifies
npm run deploy:release -- --target=staging               # pushes deploy/staging
npm run preflight                                        # check an env without booting
```

Three things about it that are easy to get wrong:

**An artifact belongs to exactly one origin.** `next build` prerenders the
localised pages and the sitemap, so `SITE_URL` is baked into 74 files. The
comment in `lib/seo/site.js` describing it as a request-time variable is true for
`robots.txt` and false for everything prerendered. A staging artifact promoted to
production would publish staging canonicals; the app refuses to start when the
running `SITE_URL` disagrees with the one it was built for.

**The app now refuses to boot on a bad production environment** rather than
serving a quietly wrong site — missing `SITE_URL`, missing `DB_*`, a `MEDIA_ROOT`
that is relative or unwritable. `lib/deploy/env-check.js` holds the rules and the
reasoning; `instrumentation.js` runs them at start. It never fires during
`next build` or in development. There is an escape hatch, deliberately awkward,
documented in the runbook.

**Never deploy, never SSH to the production host, never push to the server**
without the client saying go in the moment. Approval for one deploy is not
approval for the next. Staging first, always.
