...
| REQ-3 | Twelve functional widgets become placeable, configurable, previewable blocks | W1.30 | [x] all twelve registered plus request-form, sitemap-list, toll-matrix, toll-calculator (41 types); count and disk-coverage asserted by tests |
| REQ-7 | Provenance notices ("Not yet published", "Previous website information") are a block with a tone field, not a hand-typed `db-pending` class inside prose (catalogue finding 7) | callout + 22 | [x] `callout` block; 22-callout-migration.sql converts all 15 seeded rich-text markers, located by page slug + marker text (never id/sort); fresh-import rehearsal `tests/db/fresh-import.test.js` asserts zero markers remain after a double import |
| REQ-9 | Every admin validation message must reach the operator in a PRODUCTION build (Next redacts thrown Server Action messages; verified: bare 500) | — | [x] `lib/admin/run-action.js` wraps all 52 throw-style actions: redirect back to the form with `?notice=`, rendered by `AdminNotice`; `app/admin/(dash)/error.jsx` for genuine faults; production-verified on three screens |
| REQ-8 | Every numbered SQL file must import correctly onto a database built from 02-seed.sql, whose page/block ids differ from the development database | — | [x] 18 and 20 rewritten to resolve pages by slug (were keyed on dev-only ids 1/13/171 — would have failed or mis-targeted in production); `tests/db/fresh-import.test.js` builds from the SQL files alone, twice, and asserts each file's promise |
