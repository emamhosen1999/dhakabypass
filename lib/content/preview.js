...
export function withDraftTranslations(blocks) {
  return (blocks || []).map((block) => ({
    ...
    translations: (block.translations || []).map((t) => {
      // A live block's working copy is what the editor wants to see (W7.2).
      if (t.draft != null) return { ...t, data: t.draft, status: 'published' };
      return t.status === 'draft' ? { ...t, status: 'published' } : t;
    }),
  }));
}
