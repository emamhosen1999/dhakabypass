...
export async function getPageForAdmin(id) {
  const rows = await query(
    'SELECT id, slug, parent_id, status, published_at, updated_at, updated_by, owner_department, reviewed_at, review_interval_days FROM pages WHERE id = ? LIMIT 1',
    [Number(id)],
  );
  const page = rows?.[0];
  if (!page) return null;
...
export function parsePageSettings(formData, current) {
  ...
  return { slug, status, parentId, translations, ownerDepartment, reviewedAt: reviewedRaw || null, reviewIntervalDays };
}
  ...
  return withTransaction(async (q) => {
    ...
    await q(
      ...
      [input.slug, input.parentId, input.status, String(actor).slice(0, 191),
        input.ownerDepartment || '', input.reviewedAt, input.reviewIntervalDays, input.status, page.id],
    );
    for (const locale of LOCALES) {
