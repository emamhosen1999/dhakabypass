...
/**
 * Slugs that are `pages` rows but not URLs.
 *
 * `home` renders at `/[locale]`; `not-found` renders wherever a URL misses,
 * through app/[locale]/not-found.jsx. Neither answers at `/[locale]/<slug>`
 * — the catch-all 404s them (home permanently redirects to the root) — and
 * neither belongs in the sitemap. Named once here so the renderer, the
 * sitemap and the admin agree on the list.
 */
export const HOME_SLUG = 'home';
export const NOT_FOUND_SLUG = 'not-found';
export const RESERVED_SLUGS = Object.freeze([HOME_SLUG, NOT_FOUND_SLUG]);
