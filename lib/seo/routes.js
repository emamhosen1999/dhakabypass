...
/**
 * The inverse of localisedPath: a real URL path -> the page slug.
 *
 * `stripLocale('/bn/travel/toll')` -> `travel/toll`; `stripLocale('/zh')` -> `home`.
 * A path that carries no locale is returned as a slug unchanged.
 */
export function stripLocale(path) {
  const parts = String(path || '').split(/[?#]/)[0].replace(/^\/+|\/+$/g, '').split('/');
