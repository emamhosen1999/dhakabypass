...
import { ORG_NAME, ORG_SHORT_NAME, DEFAULT_LOGO_PATH } from './organization.js';
...
export const SEO_KEYS = {
  ...
  // The footer's copyright line, per language; {year} is replaced. Blank
  // composes "© {year} {short name}. All rights reserved." from the strings.
  copyright: 'seo.copyright_line',
  // Google Search Console's HTML-tag verification token (the content= value).
  siteVerification: 'seo.google_site_verification',
  // Bing Webmaster Tools' equivalent, published as <meta name="msvalidate.01">.
  bingVerification: 'seo.bing_site_verification',
  logoPath: 'seo.logo_path',
  // W1.10: the picture in the site header. Blank = DBEDC's own emblem,
  // cropped from the logo file DBEDC supplied (public/brand/dbedc-mark.webp),
...
export const SEO_DEFAULTS = Object.freeze({
  // Neutral: no claim and no figure (audit 2.3). The real title is a
  // setting; this is what an empty setting yields.
  siteTitle: 'Dhaka Bypass Expressway',
  siteDescription:
    'Official website of the Dhaka Bypass Expressway: toll rates, open sections, traffic and travel information.',
  favicon: '/favicon.ico',
  ogImage: '',
  robotsMode: 'default',
  robotsDisallow: [],
  ...
  headerLogo: '/brand/dbedc-mark.webp',
});
...
/** The one path under `/api/` that is published for people to use (W9.4). */
export const OPEN_DATA_PREFIX = '/api/public/';
...
export async function getSeoSettings(locale = DEFAULT_LOCALE) {
  ...
  return {
    ...
    copyright: localeText(stored(SEO_KEYS.copyright), locale) || '',
    siteVerification: asString(stored(SEO_KEYS.siteVerification)) || '',
    bingVerification: asString(stored(SEO_KEYS.bingVerification)) || '',
    logoPath: publishablePath(stored(SEO_KEYS.logoPath)) || SEO_DEFAULTS.logoPath,
    headerLogo: publishablePath(stored(SEO_KEYS.headerLogo)) || SEO_DEFAULTS.headerLogo,
  };
}
...
export function robotsRulesFor({ siteOrigin, isAdminHost, seo = SEO_DEFAULTS }) {
  ...
  return {
    // `/api/` stays disallowed — it is NextAuth callbacks and the admin's JSON
    // — but the open-data feeds live under it and exist to be cited. A longer
    // Allow beats a shorter Disallow for every crawler that implements the
    // draft, which is all of the ones that matter.
    rules: [{ userAgent: '*', allow: ['/', OPEN_DATA_PREFIX], disallow }],
 ...
 */
export function rootMetadata(seo = SEO_DEFAULTS, { brandTitles = false } = {}) {
  ...
  const title = seo.siteTitle || SEO_DEFAULTS.siteTitle;
  const metadata = {
    // Branded only for the public site (E2). The admin and the legacy tree
    // share this function and must not inherit a template: an admin screen
    // titled "Media — Dhaka Bypass Expressway" reads as a public page.
    title: brandTitles ? { default: title, template: `%s — ${title}` } : title,
  ...
  // Search Console ownership, from /admin/settings (W8C.9). Published on every
  // page of the site, as Google's HTML-tag method expects; blank publishes nothing.
  // Ownership tokens, from /admin/settings. Google has its own field in Next;
  // Bing has no named field, so it goes through `other` as msvalidate.01.
  const verification = {};
  if (seo.siteVerification) verification.google = seo.siteVerification;
...
/**
 * A page title, ready for the locale layout's template (E2).
 *
 * The template appends the road name, which is wrong for a page that already
 * says it — the home page is titled after the road, and titles typed before
 * the template existed may carry the brand too. Those are returned as
 * `{ absolute }`, which Next renders without the suffix.
 */
export function brandedTitle(title, siteTitle) {
  const t = String(title || '');
  const brand = String(siteTitle || '').trim().toLowerCase();
  if (!brand) return t;
  return t.trim().toLowerCase().includes(brand) ? { absolute: t } : t;
}
