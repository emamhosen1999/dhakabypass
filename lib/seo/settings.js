...
export async function getSeoSettings(locale = DEFAULT_LOCALE) {
  ...
  for (const row of rows || []) {
    try {
      // Already parsed by MySQL's driver, JSON text from MariaDB's: parse only
      // what still looks like JSON text, so a plain string ("DBEDC", a path)
      // survives both (lib/settings.js has the same rule).
      const v = row.value;
      raw.set(row.setting_key, typeof v === 'string' && /^\s*[[{"]/.test(v) ? JSON.parse(v) : v);
    } catch {
