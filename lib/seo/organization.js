...
export function organizationJsonLd(identity = {}) {
  ...
  return {
    ...
    ...(Array.isArray(identity.sameAs) && identity.sameAs.length
      ...
      : {}),
    // The numbers a road user needs, as the settings hold them (concession
    // audit CON-SEO-D-01): the control room as emergency, the office as
    // customer service. Either is omitted while its setting is blank.
    ...(Array.isArray(identity.contactPoints) && identity.contactPoints.length ? { contactPoint: identity.contactPoints } : {}),
    ...(identity.address ? { address: { '@type': 'PostalAddress', streetAddress: identity.address, addressCountry: 'BD' } } : {}),
  };
}

/** The site itself, with its search box, so a search engine can offer a sitelinks search. */
export function webSiteJsonLd(locale = 'en', name = ORG_NAME) {
  return {
    '@context': 'https://schema.org',
    '@type': 'WebSite',
    ...
    name,
    inLanguage: { en: 'en', bn: 'bn', zh: 'zh-Hans' }[locale] || 'en',
...
/** The page's place under its parent, from the pages table. */
...
/** An FAQ block as FAQPage. */
export function faqJsonLd(items) {
  const qa = (items || []).filter((it) => it && it.question && it.answer);
  ...
  return {
    ...
    mainEntity: qa.map((it) => ({
      '@type': 'Question',
      name: strip(it.question),
      acceptedAnswer: { '@type': 'Answer', text: strip(it.answer) },
    })),
  };
}
...
/**
 * The toll plazas as `Place` nodes (E4).
 *
 * A driver searching for "toll plaza near Gazipur" is asking a mapping
 * question, and the coordinates have been in `interchanges` since the first
 * schema while nothing published them.
 *
 * Three rules, each of which returns nothing rather than a weak claim: only a
 * plaza that is OPEN (a planned one is not somewhere a driver can stop), only
 * one with a coordinate (a Place with no geo says nothing), and only one with
 * a name in a language. `null` when none qualifies, so the caller renders no
 * empty list.
 */
const coord = (v) => (v === null || v === undefined || v === '' ? NaN : Number(v));

/**
 * A name from a locale map, English as the fallback. The same rule as
 * lib/corridor/interchanges.js localeText, restated in three lines rather
 * than imported: that module reaches the database driver, and this one is in
 * the import graph of every block on the site.
 *
 * Own properties only — a locale map is data, and a key like "constructor"
 * must not resolve up the prototype chain.
 */
const nameFor = (map, locale) => {
  const m = map || {};
  if (Object.hasOwn(m, locale) && m[locale]) return m[locale];
  if (Object.hasOwn(m, 'en') && m.en) return m.en;
  return '';
};

export function placesJsonLd(interchanges, locale = 'en') {
  const open = (interchanges || [])
    .filter((row) => row && row.kind === 'toll_plaza' && row.status === 'open')
    // Number(null) is 0, which is a valid-looking coordinate in the Gulf of
    // Guinea. A missing coordinate has to fail the check, not pass it.
    .map((row) => ({ row, name: nameFor(row.names, locale), lat: coord(row.lat), lng: coord(row.lng) }))
    .filter(({ name, lat, lng }) => name && Number.isFinite(lat) && Number.isFinite(lng))
    .sort((a, b) => Number(a.row.chainage_m || 0) - Number(b.row.chainage_m || 0));
  ...
  return {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    itemListElement: open.map(({ row, name, lat, lng }, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      item: {
        '@type': 'Place',
        '@id': `${absoluteUrl('/')}#plaza-${row.id}`,
        name,
        geo: { '@type': 'GeoCoordinates', latitude: lat, longitude: lng },
        // A toll plaza charges to pass. Saying so is the one fact about it
        // that a search engine can act on.
