...
import { OPEN_DATA_ENDPOINTS } from '../open-data/format.js';
import { NOT_FOUND_SLUG } from '../content/slug.js';
 ...
 *   listed.
 */
export function buildSitemap({ pages = [], news = [], noindex = [], pagesReadFailed = false } = {}) {
  const entries = [];
  const seen = new Set();
  ...
  // The 404 document is a published row with no URL of its own: it renders
  // wherever a URL misses, and /[locale]/not-found itself 404s. Listing it
  // would submit a URL that answers 404 — a Search Console error.
  const published = pages.filter((p) => p && p.status === 'published' && p.slug !== NOT_FOUND_SLUG);
  ...
  // Essential content routes — the travel pages — survive an OUTAGE, and only
  // an outage (audit 4.7). When the pages read succeeded, what it returned is
  // the truth: an operator who unpublished /gallery has taken it out of the
  // sitemap too. A read that failed, or that returned no published page at
  // all (which a live site never does — home is always published), is the
  // outage path, and this keeps the toll page listed through it.
  if (pagesReadFailed || published.length === 0) {
  ...
  // The open-data feeds (E7). One URL each, with no hreflang: the JSON
  // carries all three languages inside it and the calendar takes ?lang=, so
  // there is nothing to localise. Listed at all because robots.txt now allows
  // /api/public/ and a feed nobody can discover cannot be cited.
  for (const feed of OPEN_DATA_ENDPOINTS) {
    if (isHidden(feed.path)) continue;
    const url = absoluteUrl(feed.path);
    if (seen.has(url)) continue;
    seen.add(url);
    entries.push({ url, changeFrequency: 'daily', priority: 0.4 });
  }
