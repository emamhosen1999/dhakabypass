...
export async function loadOrganization() {
  ...
  try {
    ...
    sameAs = Object.values(details.social || {});
    if (details.emergency) contactPoints.push({ '@type': 'ContactPoint', contactType: 'emergency', telephone: details.emergency, availableLanguage: ['en', 'bn'], hoursAvailable: '24/7' });
    if (details.phone) contactPoints.push({ '@type': 'ContactPoint', contactType: 'customer service', telephone: details.phone, ...(details.email ? { email: details.email } : {}), availableLanguage: ['en', 'bn', 'zh'] });
    address = details.address || '';
  } catch {
    sameAs = [];
  }
  return organizationJsonLd({ name: seo.orgName, alternateName: seo.orgShortName, logo, sameAs, contactPoints, address });
}
