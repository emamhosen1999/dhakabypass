...
export async function setUiString(fullKey, locale, value) {
  assertLocale(locale);
  await keepWordingHistory(fullKey);
  await query(
...
/** The key's overrides as they stand, into history, when an operator changes them. */
async function keepWordingHistory(fullKey) {
  if (!currentActor()) return;
  const { recordHistory, logAudit } = await import('../admin/history.js');
  const snap = await recordHistory('ui_string', fullKey);
  await logAudit({ action: snap ? 'ui_string.update' : 'ui_string.create', type: 'ui_string', id: fullKey, label: `wording ${fullKey}` });
}
