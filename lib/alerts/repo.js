...
export async function listBroadcasts() {
  const rows = (await query(
    'SELECT id, messages, channel, status, recipients, sent, error, sent_by, created_at, sent_at FROM alert_broadcasts ORDER BY id DESC LIMIT 50',
  )) || [];
 ...
 */
export async function sendBroadcast({ messages, channel, sentBy = '' }, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  const res = await query(
    "INSERT INTO alert_broadcasts (messages, channel, status, recipients, sent_by) VALUES (?, ?, 'queued', ?, ?)",
    [JSON.stringify(messages), channel, recipients.length, String(sentBy || '').slice(0, 191)],
  );
