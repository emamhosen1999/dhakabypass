...
export async function alertSignup(_prev, formData) {
  ...
  // `channel` travels back so the form can record which one was chosen.
  // It is 'sms' or 'whatsapp' - a choice, not a contact detail.
  return { status: mode === 'unsubscribe' ? 'unsubscribed' : 'ok', channel };
}
