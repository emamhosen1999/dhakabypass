 ...
 */
export async function listGallery({ locale = DEFAULT_LOCALE, limit = 60, offset = 0 } = {}) {
  if (!dbEnabled()) return [];
  const take = Math.min(Math.max(Number(limit) || 60, 1), 200);
  // Pages beyond the first (INT.6). Bounded like `take`: an offset from a URL
  // is the same crawler-shaped risk as a limit from one.
  const skip = Math.min(Math.max(Math.floor(Number(offset)) || 0, 0), 100000);

  // LIMIT/OFFSET take no placeholder in a prepared statement; both are
  // bounded integers by the lines above, so no caller text reaches the SQL.
...
/** How many photographs are in the gallery, for the page links. */
export async function countGallery() {
  if (!dbEnabled()) return 0;
  const rows = await query('SELECT COUNT(*) AS c FROM media WHERE in_gallery = 1');
