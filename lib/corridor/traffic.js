...
export async function getTrafficSource() {
  ...
  return ['sample', 'tomtom', 'google', 'operator'].includes(value) ? value : 'sample';
}

export async function getMonthlyTrafficSource() {
  return (await getSetting('corridor.monthly_source', 'sample')) === 'operator' ? 'operator' : 'sample';
}
...
/**
 * The same sections, with the CHAINAGES their two waypoints sit at.
 *
 * INT.2's journey time needs to know which sections a journey between two
 * chainages actually crosses, and `corridor_sections` is keyed on waypoint
 * CODES — it has no chainage of its own. The join is done here, in SQL,
 * rather than by matching codes against a second read in JavaScript, because
 * a section whose waypoint has been renamed or deleted must disappear from
 * the calculation rather than arrive with an undefined chainage: an INNER
 * JOIN drops it, and `journeyMinutes` then refuses to produce a time at all
 * because the corridor no longer tiles. That is the safe direction.
 *
 * `from_m`/`to_m` are NOT ordered here. A section may be recorded either way
 * round; the reader normalises, exactly as lib/corridor/map.js does.
 */
export async function listCorridorSectionSpans() {
  if (!dbEnabled()) return [];
  try {
    return (await query(
      `SELECT s.id, s.condition_key, s.avg_speed_kmh, s.measured_at,
              f.chainage_m AS from_m, t.chainage_m AS to_m
         FROM corridor_sections s
         JOIN corridor_waypoints f ON f.code = s.from_code
         JOIN corridor_waypoints t ON t.code = s.to_code
...
/**
 * Every monthly count, plaza rows included, oldest first: the open-data CSV.
 * The chart above wants the corridor total only; a researcher wants all of it.
 */
export async function listMonthlyTrafficAll() {
  if (!dbEnabled()) return [];
  try {
    const rows = await query('SELECT month, plaza, vehicles FROM traffic_monthly ORDER BY month, plaza');
    return (rows || []).map((r) => ({ month: String(r.month), plaza: String(r.plaza), vehicles: Number(r.vehicles) || 0 }));
  } catch {
    return [];
  }
}
