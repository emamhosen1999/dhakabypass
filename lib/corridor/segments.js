...
import { getPublishedLengthKm } from '../settings.js';
...
export async function corridorSummary() {
  const [segments, publishedLengthKm] = await Promise.all([listSegments(), getPublishedLengthKm().catch(orLog('corridor.published_length_failed', null))]);
  return {
    extent: corridorExtent(segments),
    openLength: openLength(segments),
    percentOpen: percentOpen(segments, publishedLengthKm),
    publishedLengthKm,
    segments,
  };
}
