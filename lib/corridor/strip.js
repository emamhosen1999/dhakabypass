...
import { localeName, localeConnectsTo, localeText } from './interchanges.js';
 ...
 *
 * Exported so the tests can assert against the value in force rather than
 * against a literal of their own. A test that hard-codes a number here goes on
 * passing when this constant is lowered back towards the broken 7, which is
 * exactly the regression it exists to catch.
 */
export const DEFAULT_MIN_GAP_PCT = 15;
...
/** A DATE column value (Date or 'YYYY-MM-DD…') as 'YYYY-MM-DD', or ''. */
...
export function buildStripModel({
  segments = [], interchanges = [], locale = 'en', minGapPct = DEFAULT_MIN_GAP_PCT, publishedLengthKm = null,
} = {}) {
  // The strip is as long as the corridor, not as long as the segments that
  // happen to be recorded: when the records stopped at K35, every marker from
  // K34 to K47 was clamped onto the right edge and printed on top of the
  // others (UI audit UI-VIS-02). The far end is the published length, or the
  // furthest interchange, whichever is greater.
  const recorded = corridorExtent(segments);
  const published = Number(publishedLengthKm);
  const farthest = Math.max(
    recorded.to_m,
    Number.isFinite(published) && published > 0 ? published * 1000 : 0,
    ...(interchanges || []).map((i) => Number(i?.chainage_m) || 0),
  );
  const extent = { from_m: recorded.from_m, to_m: farthest, length_m: farthest - recorded.from_m };
  ...
  const bands = (segments || []).map((s) => {
    ...
    return {
      ...
      // The opening date: actual for an open segment, expected otherwise.
      // YYYY-MM-DD or '' — formatted for the reader by the component.
      openingDate: isoDay(s.opened_on),
    };
  });

  // Survey waypoints are geometry, not destinations. They carry coordinates so
  // the corridor map can draw the polyline, but a driver cannot use one and its
  // name ("Waypoint 4") tells them nothing -- and six of them were forcing the
  // label rows from three to six, crowding out the toll plazas that matter.
  // Filtered by KIND, never by name: a name-based filter would start publishing
  // these the moment DBEDC supplies a real name for one.
  const visible = (interchanges || []).filter((i) => i && i.kind !== 'waypoint');

  const placed = sortByChainage(visible).map((i) => ({
    id: i.id,
    name: localeName(i, locale),
    kind: i.kind,
    status: i.status,
    connectsTo: localeConnectsTo(i, locale),
