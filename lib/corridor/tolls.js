 ...
 */
/**
 * The gazette citation columns arrived in 31-cms-consistency.sql. A database
 * that has not been given that file yet answers ER_BAD_FIELD_ERROR, and the
 * schedule must not vanish over a missing citation: the read retries without
 * them (boot-check reports the missing file separately).
 */
const SRO_COLUMNS = 'sro_number, sro_date, sro_link';
async function withSro(run) {
  try {
...
export async function listTollRates({ on = new Date() } = {}) {
  ...
  const rows = (await withSro((sro) => query(
    `SELECT t.id, t.vehicle_class, t.class_labels, t.class_order, t.section,
            t.amount_bdt, t.effective_from, t.payment_methods${sro ? ', t.sro_number, t.sro_date, t.sro_link' : ''}
     FROM toll_rates t
     JOIN (
...
export async function listAllTollRates() {
  const rows = (await withSro((sro) => query(
    `SELECT id, vehicle_class, class_labels, class_order, section, amount_bdt,
            effective_from, payment_methods${sro ? `, ${SRO_COLUMNS}` : ''}
...
export async function saveTollRate({
  ...
  sro_number = '', sro_date = '', sro_link = '', payment_methods = [],
}) {
  ...
  const sroNumber = String(sro_number || '').trim();
  const sroLink = String(sro_link || '').trim();
  if (sroLink && !/^(https:\/\/|\/)/i.test(sroLink)) {
    throw validationError('The notification link must start with https:// or be a site path such as /uploads/sro.pdf');
  }
  const params = [
    vehicle_class, JSON.stringify(class_labels || {}), Number(class_order) || 0,
    section, amount, effective_from, sroNumber, String(sro_date || '').trim(), sroLink,
    JSON.stringify((Array.isArray(payment_methods) ? payment_methods : []).map((m) => String(m).trim()).filter(Boolean).slice(0, 12)),
  ];
  if (rowId !== null) {
    const res = await query(
      `UPDATE toll_rates SET vehicle_class=?, class_labels=?, class_order=?, section=?,
       amount_bdt=?, effective_from=?, sro_number=?, sro_date=?, sro_link=?, payment_methods=? WHERE id=?`,
      [...params, rowId]
    );
    if (!res.affectedRows) {
