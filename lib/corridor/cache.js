...
import { listTollOdRates } from './toll-matrix.js';
import { activeAdvisories, scheduledAdvisories } from './advisories.js';
import { isDataIllustrative, getPublishedLengthKm } from '../settings.js';
import { listCorridorRoads } from './roads.js';
...
/**
 * The origin-destination fare matrix (INT.1). Same tag and same recovery floor
 * as the flat rates it sits beside: the two are read together on a toll page
 * and an operator who corrects one expects both to refresh.
 */
export const getScheduledAdvisoriesCached = cache(() =>
  unstable_cache(() => scheduledAdvisories(), ['corridor-advisory-schedule'], { tags: [ADVISORY_TAG], revalidate: 60 })()
);

export const getTollMatrixCached = cache(() =>
  unstable_cache(() => listTollOdRates(), ['corridor-toll-matrix'], { tags: [CORRIDOR_TAG], revalidate: 300 })()
);
...
export const getCorridorRoadsCached = cache(() =>
  unstable_cache(() => listCorridorRoads(), ['corridor-roads'], { tags: [CORRIDOR_TAG], revalidate: 300 })()
);
