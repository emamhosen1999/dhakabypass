...
import { resolveRoad, roadKey } from './road-names.js';
import { buildCorridorMap, projectMercator } from './map.js';
import { t } from '../i18n/ui.js';
import { CONDITION_COLOUR } from './conditions.js';
...
/**
 * The ramp lives in ./conditions.js, which the `traffic-status` block also
 * reads. It used to be declared here; a second copy for the block would have
 * let the map and the status table beside it colour the same section
 * differently, with nothing in the suite to catch it.
 */
const CONDITION_STROKE = CONDITION_COLOUR;
...
export function buildMapView({
  ...
  locale = 'en', km, waypointName, roads = [],
}) {
  ...
  const geography = {
    image: context.image, overviewImage: '/maps/corridor-geography.avif', download: context.download, date: context.date,
    ...
    // Names come from the corridor_roads record in the reader's language
    // (audit 2.5/2.6), OpenStreetMap's own name when there is none.
    highlights:context.highlights.map((r,i)=>({id:r.id+'-'+i,roadId:roadKey(r),kind:r.kind,name:resolveRoad(r,roads,locale).name,major:r.major,d:pathOf(r.coordinates)})),
    junctions:context.junctions.map(j=>({...place(j.lng,j.lat),kind:j.kind,name:resolveRoad(context.highlights.find(h=>h.name===j.name)||{name:j.name},roads,locale).name})),
  };
  const roadGroups = new Map();
  for (const road of context.highlights) {
    const id = roadKey(road);
    if (!roadGroups.has(id)) {
      const reference = resolveRoad(road, roads, locale);
      ...
      roadGroups.set(id, { id, ref: road.ref || '', name: reference.name,
  ...
  return {
    ...
    // The termini come from the waypoint records, not the string (audit 2.7).
    alt: t(locale, 'mapAltText')
      .replace('{from}', nameOf(map.waypoints.find((w) => w.terminal)?.code ?? map.waypoints[0]?.code ?? ''))
      .replace('{to}', nameOf([...map.waypoints].reverse().find((w) => w.terminal)?.code ?? map.waypoints[map.waypoints.length - 1]?.code ?? '')),
  };
}
