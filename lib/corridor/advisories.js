...
/**
 * The public schedule (W4.8): advisories in force now and those that start
 * later, soonest first — closures and roadworks a driver can plan around.
 * An advisory whose window has ended, or that an operator switched off, is
 * not published.
 */
export async function scheduledAdvisories({ at = new Date() } = {}) {
  const now = sqlDateTime(at);
  const rows = (await query(
    `SELECT id, severity, messages, starts_at, ends_at, is_active
     FROM advisories
     WHERE is_active = 1 AND (ends_at IS NULL OR ends_at >= ?)
     ORDER BY (starts_at IS NOT NULL AND starts_at > ?), starts_at, id`,
    [now, now]
  )) || [];
  return rows.map((r) => ({ ...shape(r), upcoming: Boolean(r.starts_at) && sqlString(r.starts_at) > now }));
}
