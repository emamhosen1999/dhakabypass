 ...
 */
export function tollPoints(interchanges, { openOnly = true } = {}) {
  ...
  return interchanges
    .filter((r) => isPlainObject(r) && r.kind === 'toll_plaza'
      ...
      // A plaza that is not open to traffic is not a place a fare can be
      // charged, so the public calculator and matrix never offer it (audit
      // CON-TOLL-01: the site priced journeys to unbuilt plazas). The admin
      // passes openOnly:false to enter fares ahead of an opening.
      && (!openOnly || r.status === undefined || r.status === 'open'))
    .slice()
