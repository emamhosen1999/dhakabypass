...
/**
 * Rows we can safely compute with. A malformed row is SKIPPED, not thrown on
 * and not allowed to poison the arithmetic: percentOpen feeds a public page,
 * where one bad database row must degrade the figure, never crash the render
 * or print "NaN%".
 */
function usable(segments) {
  return list(segments).filter(
    (s) =>
      s &&
      typeof s === 'object' &&
      Number.isFinite(s.from_m) &&
      Number.isFinite(s.to_m) &&
      s.to_m > s.from_m
  );
}

export function corridorExtent(segments) {
  const rows = usable(segments);
 ...
 */
export function percentOpen(segments, publishedLengthKm = null) {
  // Against the published corridor length when DBEDC has set one: the
  // recorded segments may not yet reach the far end, and a figure computed
  // over the recorded part alone overstates progress (audit CON-OPS-01).
  const published = Number(publishedLengthKm);
  const length_m = Number.isFinite(published) && published > 0 ? published * 1000 : corridorExtent(segments).length_m;
  if (length_m <= 0) return 0;
  const pct = Math.round((openLength(segments) / length_m) * 1000) / 10;
...
export function overlaps(a, b) {
  if (
    !a ||
    !b ||
    !Number.isFinite(a.from_m) ||
    !Number.isFinite(a.to_m) ||
    !Number.isFinite(b.from_m) ||
    !Number.isFinite(b.to_m)
  ) {
    return false;
  }
