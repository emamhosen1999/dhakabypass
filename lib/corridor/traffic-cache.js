...
import { listTrafficHistory } from './history.js';
import {
...
/**
 * The centreline alone, for the kilometre-post finder. The map's reader
 * carries it too, along with twelve months of counts the finder never shows.
 */
export const getCorridorGeometryCached = cache(() => unstable_cache(
  () => listCorridorGeometry(),
  ['corridor-geometry-line'],
  { tags: [CORRIDOR_TAG], revalidate: 300 },
)());

/**
 * The kept measurements for the "typical speed by hour" block. Keyed on the
 * window so two blocks asking for 30 and 90 days do not share one entry.
 * Half an hour, because the refresh writes every half hour and the picture
 * changes by one row per section per run.
 */
export const getTrafficHistoryCached = cache((days) => unstable_cache(
  () => listTrafficHistory({ days }),
  ['corridor-traffic-history', String(days)],
  { tags: [CORRIDOR_TAG], revalidate: 1800 },
)());
