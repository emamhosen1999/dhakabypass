...
export function buildCorridorMap({
  ...
  const offX = fixedFrame ? (width - spanX * scale) / 2 : padding + chainageGutter;
  const offY = padding;
   ...
   */
  const frame = {
    w: fixedFrame ? width : spanX * scale + padding * 2 + labelGutter + chainageGutter,
    h: fixedFrame ? height : spanY * scale + padding * 2,
  };
  ...
  const featureRows = points
    .filter((p) => p.id != null && !p.isWaypoint && p.kind !== 'waypoint' && !onWaypoint.has(p.chainage))
    .map((p) => ({
  ...
  return {
    ...
    hasCentreline,
    projection: { x: minX, y: minY, scale, offX, offY },
    linePoints: linePoints.map(p => ({ x: p.x, y: p.y })),
