...
export async function saveSources(form) {
  ...
  if (!['sample', 'operator', 'tomtom', 'google'].includes(traffic) || !['sample', 'operator'].includes(monthly)) {
    throw validationError('Choose sample or operator. A live feed is enabled only by a successful refresh.');
  }
  ...
  await withTransaction(async (q) => {
    if(traffic==='tomtom' || traffic==='google') {
      ...
      if(current[0]?.value!==JSON.stringify(traffic))throw validationError('A live traffic feed can be enabled only by a successful refresh.');
    }
