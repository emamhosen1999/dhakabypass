...
export async function syncSections(q) {
...
export async function deleteWaypoint(id) {
  return withTransaction((q) => deleteWaypointWithin(q, id));
}

/** The same delete inside a transaction the caller owns (the trash's). */
export async function deleteWaypointWithin(q, id) {
...
/**
 * After a waypoint comes back from the trash: rebuild the sections, then put
 * back the last measurements of the sections that existed before it went.
 */
export async function afterWaypointRestore(q, snap) {
  await syncSections(q);
  for (const s of snap?.rows?.corridor_sections || []) {
    await q(
      `UPDATE corridor_sections SET condition_key = ?, avg_speed_kmh = ?, measured_at = ?
        WHERE from_code = ? AND to_code = ? AND measured_at IS NULL`,
      [s.condition_key || 'unknown', s.avg_speed_kmh ?? null, s.measured_at ?? null, s.from_code, s.to_code],
    );
  }
}
