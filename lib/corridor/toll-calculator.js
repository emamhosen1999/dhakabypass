...
export function calculate({ rates, interchanges, sections = [], selection } = {}) {
  ...
  // EVERY priced plaza, open or not. W8C.1 hid the plazas still under
  // construction, and the calculator shrank from six sites to two — the four
  // between Purbachal and Madanpur are exactly the ones a driver planning for
  // the opening asks about. They are offered, labelled "not yet open", and a
  // fare that touches one carries a notice saying so; the audit's objection
  // (CON-TOLL-01) was a fare presented as chargeable today, which this is not.
  const plazas = tollPoints(interchanges, { openOnly: false })
    .map((p) => ({ ...p, open: p.status === undefined || p.status === 'open' }));
  ...
  return {
    ...
    // One end not yet open to traffic: the fare is the notified rate for
    // when it opens, and the answer says so.
    unopened: !origin.open || !destination.open,
