...
/** The corridor weather reading and the thresholds that grade it. Fired when
 *  an operator changes a threshold, so the advisory follows the new value at once. */
export const WEATHER_TAG = 'weather';

export function revalidateWeather() {
  revalidateTag(WEATHER_TAG);
}
...
/**
 * The staff list. Read on every admin request (auth.js re-derives whether an
 * address may sign in), so it is cached; fired on every change at
 * /admin/users so that removing someone takes effect on their next request,
...
/** The admin's analytics snapshots (one nightly write, read by one screen). */
export const INSIGHTS_TAG = 'insights';

export function revalidateInsights() {
