...
/** The containers an implicit close may not reach across. */
const PARAGRAPH_WALLS = ['li', 'blockquote', 'td', 'th', 'figure', 'dd', 'dt'];
const SCOPE = {
  li: ['ul', 'ol'], dt: ['dl'], dd: ['dl'],
  p: PARAGRAPH_WALLS, h2: PARAGRAPH_WALLS, h3: PARAGRAPH_WALLS, h4: PARAGRAPH_WALLS,
};
...
export function sanitizeHtml(input) {
  ...
  // An implicit close only reaches an open tag of the same kind in the same
  // container: a new <li> closes the open <li> of ITS list, never the <li>
  // that holds the list, which is what used to flatten every nested list.
  const closeSame = (tag) => {
    const at = stack.lastIndexOf(tag);
    if (at === -1) return false;
    const walls = SCOPE[tag] || [];
    if (stack.slice(at + 1).some((open) => walls.includes(open))) return false;
    return closeThrough(tag);
  };
  ...
  while (i < input.length) {
    ...
    if (CLOSES_SAME.has(tag)) closeSame(tag);
    if (BLOCK.has(tag)) closeSame('p');
