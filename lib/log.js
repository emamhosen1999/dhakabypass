...
/**
 * A `.catch()` handler for the places that degrade rather than fail: a
 * dashboard panel, a public feed, an optional lookup. It returns the fallback
 * the caller wants and records why it was needed, which a bare
 * `.catch(() => [])` cannot.
 */
export const orLog = (event, fallback, fields = {}) => (err) => {
