...
/**
 * Does `value` actually show the picture at `path`?
 *
 * Built ON swapMediaPath rather than beside it, deliberately. Answering this
 * question needs exactly the matching rules above — whole-value equality plus
 * delimiter-anchored `src` attributes — and a second implementation would
 * drift from them the first time either is touched. A substring test would
 * say yes for prose that merely names the file and for `/photo/20.webp` when
 * asked about `/photo/2.webp`; both of those are the corruption swapMediaPath
 * is careful to avoid, and they matter here too because the answer decides
 * which pages get revalidated when a description changes.
 *
 * The sentinel it swaps to can never equal `path`, so a match is a match for
 * the same reason the rewriter would have rewritten it. Nothing is mutated:
 * swapMediaPath copies on the way out and the copy is discarded.
 */
export function referencesMediaPath(value, path) {
  if (typeof path !== 'string' || !path) return false;
  return swapMediaPath(value, path, `${path}\u0000`).changed > 0;
}
