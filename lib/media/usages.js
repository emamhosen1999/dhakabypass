...
export async function mediaUsageMap(q, paths) {
  ...
  const [blocks, og, news, settings, routes, cameras] = await Promise.all([
    ...
    q(`SELECT route, og_image FROM route_meta WHERE og_image IS NOT NULL AND og_image <> ''`).catch(orLog('media.usages.route_meta_failed', [])),
    q(`SELECT names, snapshot_url FROM cameras WHERE snapshot_url LIKE '/%'`).catch(orLog('media.usages.cameras_failed', [])),
  ]);
