 ...
 */
/** Returns the migration problem entry, or null when the ledger is complete. */
async function checkMigrations(readMigrations) {
  const { missingMigrations, migrationProblem, readAppliedMigrations } = await import('../db/migrations.js');
  let ledger;
  if (readMigrations) {
    ledger = await readMigrations();
  } else {
    const { query, dbEnabled } = await import('../db.js');
    if (!dbEnabled()) return null; // reported by checkEnvironment already
    ledger = await readAppliedMigrations(query);
  }
  return migrationProblem(missingMigrations(ledger.applied), ledger.error);
}
...
export async function assertBootEnvironment({
  ...
  // The ledger reader (W6.6). Injected so the check is testable without a
  // database; the default reads through lib/db.js, which is imported lazily
  // because this module also runs in `next build`'s workers.
  readMigrations = null,
} = {}) {
  ...
  // The schema gate (W6.6): every SQL file this build expects must be in the
  // database's ledger. Only once the connection variables have passed — a
  // database that cannot be reached is already reported above, and asking it
  // for the ledger would only add a second, confusing line.
  if (!result.problems.some((p) => String(p.key).startsWith('DB'))) {
    const migrationProblem = await checkMigrations(readMigrations);
    if (migrationProblem) result.problems.push(migrationProblem);
  }
