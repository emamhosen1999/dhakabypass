...
export async function getNewsTranslations(newsId) {
  ...
  const rows = await query(
    `SELECT news_id, locale, title, excerpt, body, status, draft, updated_at
       FROM news_translations WHERE news_id = ?`,
    [Number(newsId)],
  );
  return (rows || []).map((r) => ({ ...r, draft: r.draft == null ? null : (typeof r.draft === 'string' ? JSON.parse(r.draft) : r.draft) }));
}
...
export async function saveNewsTranslation({ newsId, locale, title, excerpt, body, status }) {
  // A draft of a published translation is kept beside it (W7.2): the public
  // article keeps its live text until the draft is published.
  if (status === 'draft') {
    const rows = await query('SELECT status FROM news_translations WHERE news_id = ? AND locale = ? LIMIT 1', [Number(newsId), locale]);
    if (rows?.[0]?.status === 'published') {
      await query(
        'UPDATE news_translations SET draft = ?, draft_updated_at = CURRENT_TIMESTAMP WHERE news_id = ? AND locale = ?',
        [JSON.stringify({ title, excerpt, body }), Number(newsId), locale],
      );
      return 'draft_kept';
    }
  }
  await query(
    `INSERT INTO news_translations (news_id, locale, title, excerpt, body, status)
     ...
     ON DUPLICATE KEY UPDATE
       ...
       body = VALUES(body), status = VALUES(status), draft = NULL, draft_updated_at = NULL`,
