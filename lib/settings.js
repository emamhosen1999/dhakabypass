...
// Every reader asks for CAST(value AS CHAR): MySQL's driver would otherwise
// hand a JSON column back already parsed while MariaDB's hands back text, and
// a stored string such as "999" would reach JSON.parse as the bare 999.
...
export async function setSetting(key, value) {
  ...
  if (currentActor()) {
    ...
    const rows = await query('SELECT CAST(value AS CHAR) AS value FROM site_settings WHERE setting_key = ? LIMIT 1', [key]);
...
export async function getContactDetails(locale = DEFAULT_LOCALE) {
  ...
  try {
    rows = await query(
      `SELECT setting_key, CAST(value AS CHAR) AS value FROM site_settings WHERE setting_key IN (${[...Object.values(CONTACT_KEYS), ...Object.values(SOCIAL_KEYS)].map(() => '?').join(', ')})`,
      [
