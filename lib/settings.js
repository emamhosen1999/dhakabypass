import { currentActor } from './admin/context.js';
import { query } from './db.js';
import { DEFAULT_LOCALE } from './i18n/locales.js';
import { isPlainObject } from './json.js';

// A JSON column arrives already parsed from MySQL's driver but as text from
// MariaDB's. A stored string such as "999" therefore reaches us either as the
// bare string 999 (parse would make it a number, and "+880 2…" would throw)
// or as the JSON text "\"999\"". Parse only what still looks like JSON text.
const asJson = (v) => {
  if (typeof v !== 'string') return v;
  const t = v.trim();
  // Numbers and booleans come back from both drivers as their own types;
  // a numeric-looking STRING is a string (a phone number, a road code).
  if (/^[[{"]/.test(t)) {
...
export async function setSetting(key, value) {
  // An operator's change (inside an admin action) is kept in history first; a
  // cron job's refresh is not an edit and is not recorded.
  if (currentActor()) {
    const { recordHistory, logAudit } = await import('./admin/history.js');
    const { stable, safeParse } = await import('./admin/entities.js');
    const rows = await query('SELECT value FROM site_settings WHERE setting_key = ? LIMIT 1', [key]);
    const before = rows?.[0] ? safeParse(rows[0].value) : undefined;
    if (stable(before) !== stable(value)) {
      await recordHistory('setting', key);
      await logAudit({ action: before === undefined ? 'setting.create' : 'setting.update', type: 'setting', id: key, label: `setting ${key}` });
    }
  }
  await query(
...
export async function getPublishedLengthKm() {
  ...
  // Blank (saved as null from /admin/corridor) is "no published figure", not 0.
...
export const CONTACT_KEYS = {
  ...
  // The national emergency service (999 in Bangladesh), shown beside DBEDC's
  // own line. A setting, not a literal, so it lives in one place (audit 2.1).
  nationalEmergency: 'contact.national_emergency_phone',
...
export async function getContactDetails(locale = DEFAULT_LOCALE) {
  ...
  try {
    rows = await query(
      `SELECT setting_key, value FROM site_settings WHERE setting_key IN (${[...Object.values(CONTACT_KEYS), ...Object.values(SOCIAL_KEYS)].map(() => '?').join(', ')})`,
      [
  ...
  const details = {
    ...
    nationalEmergency: asString(raw.get(CONTACT_KEYS.nationalEmergency)),
