...
export default {
  ...
  fields: [
    ...
    // The home page carries the map as a wide band with no controls — the
    // road as it is running now, and a link to the full explorer.
    {
      name: 'layout', type: 'select', label: 'Layout', default: 'full',
      options: [
        { value: 'full', label: 'Full explorer: zoom, layers, section list' },
        { value: 'compact', label: 'Compact band: the map only, with a link to the full map' },
      ],
    },
    { name: 'linkHref', type: 'text', label: 'Where “open the full map” goes (compact only; blank = travel/map)' },
    { name: 'linkLabel', type: 'text', label: 'Text of that link (blank = the built-in wording)' },
  ],
