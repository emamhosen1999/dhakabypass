 ...
 */
// `select` is a closed choice inside a row — a statistic's live source (audit
// 5.9). A value outside the declared options is stored as the first option.
export const ITEM_FIELD_TYPES = ['text', 'textarea', 'richtext', 'image', 'number', 'list', 'select'];
...
const isBlank = (row, fields) => fields.every((f) => {
  ...
  if (f.type === 'select') return !row[f.name];
  return row[f.name] === '';
});
      ...
      .map((row) => {
        ...
        for (const f of fields) {
          ...
          if (f.type === 'number') out[f.name] = Number(raw ?? 0) || 0;
            ...
            // Absent stays absent: a seeded row is kept exactly as written.
            if (raw === undefined) continue;
            const allowed = (Array.isArray(f.options) ? f.options : []).map((o) => o.value);
            out[f.name] = allowed.includes(raw) ? raw : (allowed[0] ?? '');
          }
