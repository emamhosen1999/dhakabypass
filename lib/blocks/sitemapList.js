...
export function groupSiteIndex(pages, grouped = true, labelFor = () => '') {
  ...
  for (const p of rows) {
    ...
    if (!section) {
      ...
      section = {
        key: first,
        heading: parent ? parent.title : (labelFor(first) || humanise(first)),
...
/** `land-acquisition` -> `Land acquisition`. Only used when no parent page
 *  supplies a real, translated title — a last resort, not a design. */
