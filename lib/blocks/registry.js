...
/**
 * A select whose values are RECORDS, not code (W8N.3).
 *
 * `section-subnav.menu` listed exactly one menu because its options were a
 * literal array, so an operator who created an `about` menu at /admin/menus
 * could not place it on the About page: the only second-level navigation on the
 * site was travel's, and the other five sections had none. A field that
 * declares `optionsFrom` keeps its static options as the offline fallback, and
 * the admin fills the control from the database at render time (see
 * lib/blocks/field-options.js).
 *
 * The validator then accepts any slug-shaped value rather than the frozen list.
 * It is not a hole: the renderer resolves an unknown menu to the built-in
 * links, the same override-never-replace rule the header and footer follow, so
 * the worst a stale value can do is show the navigation the page already had.
 */
const SLUG_VALUE = /^[a-z0-9][a-z0-9-]{0,63}$/;
...
export function validateBlockData(type, data) {
  ...
  for (const field of def.fields) {
    ...
    // A declared range is enforced here, so the editor hears about 0 or 5000
    // instead of the renderer clamping it silently (audit 3.4).
    if (!absent && field.type === 'number'
      && ((Number.isFinite(field.min) && value < field.min) || (Number.isFinite(field.max) && value > field.max))) {
      errors.push(`${field.label} must be between ${field.min ?? '-∞'} and ${field.max ?? '∞'}`);
      continue;
    }
    if (!absent && field.type === 'select' && field.optionsFrom) {
      if (typeof value !== 'string' || !SLUG_VALUE.test(value)) {
        errors.push(`${field.label} must be the name of a menu, in lower case`);
      }
    } else if (!absent && field.type === 'select' && !optionValues(field).includes(value)) {
      errors.push(`${field.label} must be one of: ${optionValues(field).join(', ')}`);
    }
  }
